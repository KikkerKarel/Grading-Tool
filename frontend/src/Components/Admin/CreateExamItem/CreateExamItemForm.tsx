...
class CreateExamItemForm extends Component {
    ...
    componentDidMount() {
        ...
        axios.get(`/api/question/find/all`).then(response => {
            this.setState({
                ...
                questionType: response.data[0].type
            });
        });
    }
    ...
    Input = (type: String, questionID: number) => {
        if (type === "CHOICE") {
            ...
            return (
                <>
                    ...
                    <Form.Control as="select" id="McCorrectAnswer"
                                  onChange={(e) => this.setState({mcAnswer: e.target.value.toString()})}>
                        {myChoices}
                    </Form.Control>
                </>
            )
            ...
            return (
                <Form.Control type="text" value={this.state.oqAnswer} placeholder="Open vraag antwoord" required
                              onChange={(e) => this.setState({oqAnswer: e.target.value})}/>
            )
        }
    }
        ...
        return (
            <Form onSubmit={(e) => this.SubmitForm(e)}>
                ...
                <Button variant="primary m-2" type="submit"
                        disabled={this.validateForm(this.state.questionType, this.state.oqAnswer)}>Koppelen</Button>
            </Form>
        )
    }
}
